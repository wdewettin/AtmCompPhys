...
global_attrs_path = (
    "/dodrio/scratch/users/vsc45263/wout/readFA/data/example_CORDEX_attributes.yml"
)
...
toffset = (
    rstart_datetime - datetime(rstart_datetime.year, rstart_datetime.month, 1, 0)
) // timedelta(seconds=1)
...
FA_to_CF(
    ...
    rbin=rbin,
    zero_freq=zero_freq,
    global_attrs_path=global_attrs_path,
)

# Add run_name and rstart to coordinates
for var_name in cf_var_name_list:
    file_name = os.path.join(
        cf_dir, var_name, f"{storefilebase}_{var_name}_{tstart}_{tstop}_{tstep}.nc"
    )
